...
- **2つのTTSエンジン**: MioTTS-Inference（話者クローン対応・高品質）と Kokoro TTS（高速・軽量・サーバー不要）を切り替え可能
...
> Ollama は MioTTS の LLM バックエンドとして使用します。翻訳には使用しません。Kokoro TTS モードでは不要です。
...
| `TTS_ENGINE` | TTS エンジンの選択 | `miotts` または `kokoro` |
...
| 項目 | `miotts` | `kokoro` |
|------|----------|----------|
| エンジン | MioTTS-Inference | Kokoro TTS (82M パラメータ) |
| ボイスクローン | 対応（元話者の声色を再現） | 非対応（固定ボイス） |
| 処理速度 | 低速 | 高速 |
| サーバー | 必要（Ollama + MioTTS API） | 不要（プロセス内推論） |
| 追加セットアップ | MioTTS-Inference クローン + Ollama | `uv run python -m unidic download` |
| 音質 | 話者ごとに異なる声色で高品質 | 固定ボイスだが自然な発声 |
...
MioTTS-Inference による話者クローン TTS です。元の話者の声色を再現した日本語音声を生成します。Ollama（LLM バックエンド）と MioTTS API サーバーの起動が必要です。
...
Kokoro は 82M パラメータの軽量オープンウェイト TTS モデルです。ボイスクローンには対応していませんが、高速に日本語音声を生成できます。MioTTS のようなサーバー起動は不要で、翻訳サーバー（plamo-translate-cli）のみで動作します。
...
| `KOKORO_HTTP_TIMEOUT` | `300.0` | HTTP タイムアウト（秒） |
...
本ツールは翻訳に plamo-translate-cli サーバーを使用します。TTS エンジンによって必要なサーバーが異なります。**パイプライン実行前にサーバーを起動しておく必要があります。**
...
TTS エンジンの設定に応じて、適切な起動スクリプトが自動生成されます。

- **Kokoro TTS モード**: plamo-translate-cli のみ起動
- **MioTTS モード**: plamo-translate-cli + Ollama + MioTTS-Inference の3つを起動

初回実行時に PLaMo-2-Translate の MLX モデルが自動でダウンロードされます。MioTTS モードの場合は Ollama モデルも自動ダウンロードされます。

### 方法B: 手動で起動する

#### Kokoro TTS モードの場合（ターミナル1つ）

```bash
# plamo-translate-cli 翻訳サーバー（MLX, 8bit）
uv run plamo-translate server --precision 8bit
```

#### MioTTS モードの場合（ターミナル3つ）
...
5. セグメント結合 → spaCy 文分割 → 翻訳ユニット結合（話者境界を維持）
6. MioTTS: 話者ごとのリファレンス音声を元動画から抽出 / Kokoro: 省略
7. plamo-translate-cli (PLaMo-2-Translate, MLX 8bit) で英日翻訳
8. MioTTS: 話者クローン日本語音声を生成 / Kokoro: 日本語音声を高速生成
9. 元動画の各区間を TTS 音声長に合わせて速度伸縮
10. 映像 + 日本語音声（+ 英語音声を薄くミックス）を合成して出力
...
| `TTS_ENGINE` | `miotts` | TTS エンジン（`miotts` または `kokoro`） |
...
| `KOKORO_HTTP_TIMEOUT` | `300.0` | Kokoro HTTP タイムアウト（秒） |
