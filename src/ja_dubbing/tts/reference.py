...
from ja_dubbing.utils import atomic_write_json, ensure_dir, load_json_if_exists, print_step
...
class SpeakerReferenceCache:
    ...
    def build_gptsovits_references(
        ...
        for speaker_id, dia_segments in speakers.items():
            ...
            # 目標長（5秒）に最も近い発話区間を選ぶ
            best_seg = _select_best_reference_segment(dia_segments)
            if best_seg is None:
                print_step(
...
def _select_best_reference_segment(
    ...
    """GPT-SoVITS に最適なリファレンス区間（目標5秒に最も近い）を選ぶ。"""
    candidates = []
    for seg in dia_segments:
        dur = seg.end - seg.start
        if dur < GPTSOVITS_REFERENCE_MIN_SEC:
            continue
    ...
    def score(seg: DiarizationSegment) -> float:
        dur = seg.end - seg.start
        if dur > GPTSOVITS_REFERENCE_MAX_SEC:
            dur = GPTSOVITS_REFERENCE_MAX_SEC
        return abs(dur - GPTSOVITS_REFERENCE_TARGET_SEC)
    ...
    if dur > GPTSOVITS_REFERENCE_MAX_SEC:
        return DiarizationSegment(
            start=best.start,
            end=best.start + GPTSOVITS_REFERENCE_MAX_SEC,
